...
#[derive(Clone, Copy)]
enum BigIntegerHandling {
    ApproximateForValidation,
    RejectLossySerialization,
}

fn py_to_json_value(value: &Bound<'_, PyAny>) -> PyResult<JsonValue> {
    py_to_json_value_inner(
        value,
        &mut HashSet::new(),
        BigIntegerHandling::ApproximateForValidation,
    )
}
...
fn py_to_serializable_json_value(value: &Bound<'_, PyAny>) -> PyResult<JsonValue> {
    py_to_json_value_inner(
        value,
        &mut HashSet::new(),
        BigIntegerHandling::RejectLossySerialization,
    )
}
...
fn py_to_json_value_inner(
    ...
    big_integer_handling: BigIntegerHandling,
) -> PyResult<JsonValue> {
    if value.is_none() {
    ...
    if value.is_instance_of::<PyInt>() {
        return py_int_to_json_value(value, big_integer_handling);
    }
    ...
    if let Ok(list) = value.cast::<PyList>() {
        ...
        let result = list
            .iter()
            .map(|item| py_to_json_value_inner(&item, active_containers, big_integer_handling))
    ...
    if let Ok(dict) = value.cast::<PyDict>() {
        ...
        for (key, item) in dict {
            ...
            let item = match py_to_json_value_inner(&item, active_containers, big_integer_handling)
            {
                Ok(item) => item,
                Err(error) => {
                    active_containers.remove(&container_id);
...
fn py_int_to_json_value(
    ...
    match big_integer_handling {
        BigIntegerHandling::ApproximateForValidation => {
            let rendered = value.str()?.to_str()?.to_owned();
            parse_json(&rendered)
        }
        BigIntegerHandling::RejectLossySerialization => Err(PyErr::new::<PyOverflowError, _>(
...
struct ValidatorPy {
    ...
    fast_validation_supported: bool,
}
...
impl ValidatorPy {
    ...
    fn is_valid_value(&self, instance: &Bound<'_, PyAny>) -> PyResult<bool> {
        if self.fast_validation_supported
            && let Some(valid) = fast_validation::is_valid_python(&self.schema, instance)?
            && valid
        {
            return Ok(true);
        }
    ...
    fn construct_value(
        ...
        if self.fast_validation_supported
            ...
            && valid
        {
            return converter.construct_python(py, instance).map(Some);
        }
        let (model, json_value) = match converter.construct_with_json(py, instance) {
            Ok(result) => result,
            Err(conversion_error) => {
                let json_value = py_to_json_value(instance)?;
                if !validate_value_for_schema(&self.schema, &json_value)? {
                    return Ok(None);
                }
...
impl ValidatorPy {
    fn construct_json_bytes(
        ...
        if self.fast_validation_supported
            && let Some(valid) = fast_validation::is_valid(&self.schema, &parsed)
        {
            if !valid {
                return Ok(None);
            }
        ...
        let (model, json_value) = match converter.construct_jiter_with_json(py, &parsed) {
            Ok(result) => result,
            Err(conversion_error) => {
                let json_value = jiter_to_serde_json(&parsed)?;
                if !validate_value_for_schema(&self.schema, &json_value)? {
                    return Ok(None);
                }
                return Err(conversion_error);
...
fn validator_for_schema(schema_json: &str) -> PyResult<ValidatorPy> {
    ...
    let fast_validation_supported = fast_validation::supports(&schema);
    Ok(ValidatorPy {
        schema,
        fast_validation_supported,
    })
}
