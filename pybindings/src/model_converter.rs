...
impl ModelConverterPy {
    pub(crate) fn construct_with_json(
        &self,
        py: Python<'_>,
        value: &Bound<'_, PyAny>,
    ) -> PyResult<(Py<PyAny>, JsonValue)> {
        self.convert_with_json(py, self.root, value, MAX_MODEL_DEPTH)
    }
    ...
    pub(crate) fn construct_jiter_with_json(
        &self,
        py: Python<'_>,
        value: &JiterJsonValue<'_>,
    ) -> PyResult<(Py<PyAny>, JsonValue)> {
        self.convert_jiter_with_json(py, self.root, value)
    }
    ...
    fn convert_with_json(
        ...
        match node {
            ConversionNode::Scalar {
                ...
                convert_scalar(py, *kind, missing_sentinel.as_ref(), value, false)?,
                scalar_json_value(value)?,
            )),
            ConversionNode::Literal { values } => Ok((
                convert_literal(py, values, value, false)?,
                scalar_json_value(value)?,
            )),
            ConversionNode::List { item } => {
                let input = value
                ...
                let mut json_values = Vec::with_capacity(input.len());
                for item_value in input {
                    let (converted_item, json_item) =
                        self.convert_with_json(py, *item, &item_value, remaining_depth - 1)?;
                    converted.push(converted_item);
                    json_values.push(json_item);
                }
                Ok((
            ...
            ConversionNode::Dict {
                ...
                let input = value
                    ...
                    .map_err(|_| expected_type("dict", value).unwrap())?;
                let output = PyDict::new(py);
                let mut json_object = JsonMap::with_capacity(input.len());
                for (key_value, item) in input {
                    let (converted_key, _) =
                        self.convert_with_json(py, *key, &key_value, remaining_depth - 1)?;
                    let (converted_value, json_value) =
                        self.convert_with_json(py, *value_node, &item, remaining_depth - 1)?;
                    let json_key = key_value.extract::<String>().map_err(|_| {
                        PyErr::new::<PyTypeError, _>("JSON object keys must be strings")
                    })?;
                    output.set_item(converted_key, converted_value)?;
                    json_object.insert(json_key, json_value);
                }
                Ok((output.into_any().unbind(), JsonValue::Object(json_object)))
            }
            ConversionNode::Union {
                branches,
                discriminator,
            } => self.convert_union_with_json(
                py,
                branches,
                discriminator.as_ref(),
                value,
            ...
            ConversionNode::Root {
                ...
                let (converted, json_value) =
                    self.convert_with_json(py, *value_node, value, remaining_depth - 1)?;
                let instance = allocate_model(py, model_type, &self.object_new)?;
                set_model_attribute(py, &instance, root_py_name, &converted)?;
                Ok((instance.unbind(), json_value))
            }
        }
    }

    fn convert_union_with_json(
        &self,
        py: Python<'_>,
        branches: &[usize],
        discriminator: Option<&DiscriminatorPlan>,
        ...
        if let (Some(plan), Ok(object)) = (discriminator, value.cast::<PyDict>())
            ...
            && let Some(branch) = plan.branches_by_value.get(&tag)
        {
            return self.convert_with_json(py, *branch, value, remaining_depth - 1);
        }
        ...
        if matching.len() == 1 {
            return self.convert_with_json(py, matching[0], value, remaining_depth - 1);
        }
        ...
        for branch in candidates {
            if let Ok(converted) = self.convert_with_json(py, *branch, value, remaining_depth - 1) {
                return Ok(converted);
            }
        }
        Err(PyErr::new::<PyTypeError, _>(
    ...
    fn convert_model_with_json(
        ...
        for (key, item) in input {
            ...
            if let Some(field_index) = fields_by_json_name.get(&key_string) {
                ...
                let (converted, json_value) =
                    self.convert_with_json(py, field.value_node, &item, remaining_depth - 1)?;
                converted_fields[*field_index] = Some(converted);
                json_object.insert(key_string, json_value);
            } else if let (Some(extra_node), Some(output)) = (extra_value, extra_output.as_ref()) {
                let (converted, json_value) =
                    self.convert_with_json(py, extra_node, &item, remaining_depth - 1)?;
                output.set_item(key, converted)?;
                json_object.insert(key_string, json_value);
            } else {
                json_object.insert(key_string, super::py_to_json_value(&item)?);
            }
        }
        ...
        Ok((instance.unbind(), JsonValue::Object(json_object)))
    }

    fn convert_jiter_with_json(
        &self,
        py: Python<'_>,
        node_id: usize,
        value: &JiterJsonValue<'_>,
        ...
        match node {
            ConversionNode::Scalar { kind, .. } => convert_jiter_scalar(py, *kind, value),
            ConversionNode::Literal { values } => {
                let python_value = value.into_pyobject(py)?.unbind();
                let converted = convert_literal(py, values, python_value.bind(py), false)?;
                Ok((converted, super::jiter_to_serde_json(value)?))
            }
            ConversionNode::List { item } => {
                ...
                for item_value in items.iter() {
                    let (converted_item, json_item) =
                        self.convert_jiter_with_json(py, *item, item_value)?;
                    converted.push(converted_item);
                    json_values.push(json_item);
                }
                Ok((
            ...
            ConversionNode::Dict {
                ...
                let output = PyDict::new(py);
                let mut json_object = JsonMap::with_capacity(entries.len());
                for (key_value, item) in entries.iter() {
                    let jiter_key = JiterJsonValue::Str(key_value.clone());
                    let (converted_key, _) = self.convert_jiter_with_json(py, *key, &jiter_key)?;
                    let (converted_value, json_value) =
                        self.convert_jiter_with_json(py, *value_node, item)?;
                    output.set_item(converted_key, converted_value)?;
                    if json_object
                        .insert(key_value.to_string(), json_value)
                        .is_some()
                    {
            ...
            ConversionNode::Union {
                branches,
                discriminator,
            } => self.convert_jiter_union(py, branches, discriminator.as_ref(), value),
            ConversionNode::Model {
                model_type,
                fields,
                fields_by_json_name,
            ...
            ConversionNode::Root {
                ...
                let (converted, json_value) =
                    self.convert_jiter_with_json(py, *value_node, value)?;
                let instance = allocate_model(py, model_type, &self.object_new)?;
                set_model_attribute(py, &instance, root_py_name, &converted)?;
                Ok((instance.unbind(), json_value))
            }
    ...
    fn convert_jiter_union(
        ...
        if let (Some(plan), JiterJsonValue::Object(entries)) = (discriminator, value)
            ...
            && let Some(branch) = plan.branches_by_value.get(tag.as_ref())
        {
            return self.convert_jiter_with_json(py, *branch, value);
        }
        ...
        let matching = branches
            ...
            .collect::<Vec<_>>();
        if matching.len() == 1 {
            return self.convert_jiter_with_json(py, matching[0], value);
        }
        let candidates = if matching.is_empty() {
            branches
        } else {
        ...
        for branch in candidates {
            if let Ok(converted) = self.convert_jiter_with_json(py, *branch, value) {
                return Ok(converted);
            }
        }
        Err(PyErr::new::<PyTypeError, _>(
    ...
    fn convert_jiter_model(
        ...
        for (key, item) in entries.iter() {
            ...
            if let Some(field_index) = fields_by_json_name.get(key_string) {
                let field = &fields[*field_index];
                let (converted, converted_json) =
                    self.convert_jiter_with_json(py, field.value_node, item)?;
                converted_fields[*field_index] = Some(converted);
                json_value = converted_json;
            } else if let (Some(extra_node), Some(output)) = (extra_value, extra_output.as_ref()) {
                let (converted, converted_json) =
                    self.convert_jiter_with_json(py, extra_node, item)?;
                output.set_item(key_string, converted)?;
                json_value = converted_json;
            } else {
                json_value = super::jiter_to_serde_json(item)?;
            }

            if json_object.insert(key.to_string(), json_value).is_some() {
                return Err(duplicate_key(key));
            }
        }
...
fn convert_jiter_scalar(
    ...
    Ok((
        ...
        super::jiter_to_serde_json(value)?,
    ))
}
...
fn scalar_json_value(value: &Bound<'_, PyAny>) -> PyResult<JsonValue> {
    if value.is_none() {
        return Ok(JsonValue::Null);
    }
    if value.is_instance_of::<PyBool>() {
