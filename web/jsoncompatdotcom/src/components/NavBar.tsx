...
export default function NavBar() {
  ...
  const linkClasses = (href: string) =>
    `px-3 py-2 rounded-md text-sm font-medium ${
      current === href ? "bg-blue-700 text-white" : "text-gray-300 hover:bg-blue-500 hover:text-white"
    }`;
        ...
        <div className="flex h-16 items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <div className="hidden md:flex md:items-baseline md:space-x-2">
              <Link to="/checker" className={linkClasses("/checker")}>Checker</Link>
              <Link to="/fuzzer" className={linkClasses("/fuzzer")}>Fuzzer</Link>
              <Link to="/install" className={linkClasses("/install")}>Install</Link>
            </div>
          </div>
