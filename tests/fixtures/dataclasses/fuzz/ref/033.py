...
class GeneratedSchemaFoo(jsoncompat_dataclasses.DataclassRootModel):
    __jsoncompat_schema__: typing.ClassVar[str] = "{\"$defs\":{\"foo\":{\"type\":\"number\"}},\"type\":\"number\"}"
    root: float = jsoncompat_dataclasses.jsoncompat_root_field()

@dataclass(frozen=True, slots=True, kw_only=True)
class GeneratedSchema(jsoncompat_dataclasses.DataclassRootModel):
    __jsoncompat_schema__: typing.ClassVar[str] = "{\"$defs\":{\"foo\":{\"type\":\"number\"}},\"$id\":\"file:///c:/folder/file.json\",\"$ref\":\"#/$defs/foo\",\"$schema\":\"https://json-schema.org/draft/2020-12/schema\"}"
