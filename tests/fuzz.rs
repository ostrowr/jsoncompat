...
fn remote_root() -> &'static PathBuf {
    ...
    ROOT.get_or_init(|| {
        fs::canonicalize("tests/fixtures").expect("missing tests/fixtures directory for remotes")
    })
}
...
fn start_fixture_server() {
    ...
    START.get_or_init(|| {
        let server = Server::http("127.0.0.1:1234").expect("failed to start fixture server");
        thread::spawn(move || {
